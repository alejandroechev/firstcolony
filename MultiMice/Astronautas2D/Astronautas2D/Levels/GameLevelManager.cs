        ...
        public void Load(string gameFile, GameObjectFactory gof, GUIElementFactory gef, Rectangle viewRect)
        {
            ...
            foreach (XElement levelNode in levelElements)
            {
                ...
                ciObject = tObject.GetConstructor(
                    new Type[10] 
                    { 
                        counter.GetType(), objectiveType, narrative.GetType(), this.miceManager.GetType(), gof.GetType(), gef.GetType(), viewRect.GetType(), conditions.GetType(), chargeValue.GetType(), allAtOnce.GetType()
                    }
                    );
                if (tObject == Type.GetType("Astronautas2D.Levels.SoloLevel"))
                {
                    ...
                    SoloLevel levelObj = ciObject.Invoke(new object[] { counter, objective, narrative, this.miceManager, gof, gef, viewRect, conditions, chargeValue, false }) as SoloLevel;
                ...
                else
                {
                    ...
                    TeamLevel levelObj = ciObject.Invoke(new object[] { counter, objective, narrative, this.miceManager, gof, gef, viewRect, conditions, chargeValue, allAtOnce }) as TeamLevel;
                    levelObj.File = fileName;
